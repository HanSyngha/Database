...
void _insert(BTreeNode* present, int k)
{
	...
	{
		i--;
		while( i >= 0 && present->keys[i] > k ) i--;
		_insert(present->C[i+1],k);
	}
}
...
void _remove(BTreeNode* present, int k)
{
	...
	if(present->keys[i] == k)
	{
		if(present->leaf)
		{
			for(;i< ( present->n - 1 );i++) present->keys[i] = present->keys[i+1];
			present->keys[ present->n - 1] = 0;
			present->n--;
		}
		else
		{
			temp = present->keys[i];
			present->keys[i] = present->C[0]->keys[0];
			present->C[0]->keys[0] = temp;
